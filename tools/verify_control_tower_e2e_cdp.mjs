...
const task17Scenario = args.scenario === 'task17-projection';
const fixturePath = resolve(root, String(args.fixture || 'control_tower/fixtures/two-product-mixed-manual/manifest.json'));
const evidence = resolve(root, String(args.evidence || (task17Scenario
...
const listen = (server, port = 0) => new Promise(resolveListen => server.listen(port, '127.0.0.1', () => resolveListen(server.address().port)));
...
async function runTask17() {
  ...
  const authority = startAuthority(authorityPort, statePath, eventLogPath);
  const bff = startBff(bffPort, frontPort, authorityPort, 'task17');
  ...
  const holdProjection = (jobId, sequence, order = {}) => ({
    ...
    session: {
      workspaceId: `batch:${jobId}`,
      productId: order.productId || `factory:${jobId}`,
      productKey: order.productKey || jobId,
      runId: order.currentRunId || `run:${jobId}`,
      inputFingerprint: order.command?.payload?.inputFingerprint || `sha256:${jobId}`,
      revision: order.expectedWorkfileRevision || 10,
    },
    ...
    registration: { status: 'blocked', blockers: ['final_detail_a_cut'], jobId },
  });
  const holdJob = async (base, headers, job, sequence) => {
    ...
    const projection = holdProjection(job.jobId, sequence, order);
    const completed = await lifecycle(base, headers, order, 'complete', 'task17-worker', 2, {
  ...
  try {
    ...
    await api(base, '/api/factory/session/hello', {
      ...
      body: JSON.stringify({
        ...
        sessionId: 'task17-session',
        workerId: 'task17-worker',
    ...
    const first = (await api(base, '/api/factory/jobs', {
      method: 'POST', headers, body: JSON.stringify(productPayload('a')),
    })).job;
    const second = (await api(base, '/api/factory/jobs', {
      method: 'POST', headers, body: JSON.stringify(productPayload('b')),
    })).job;
    const worker = {
      first: await holdJob(base, headers, first, 8),
      second: await holdJob(base, headers, second, 9),
    };
    await api(base, '/api/factory/session/hello', {
      method: 'POST',
      headers,
      body: JSON.stringify({
        ...
        startedAt: 1000,
        cursor: 10,
        projection: holdProjection(first.jobId, 10, worker.first.order),
      }),
    });
    const before = await api(base, '/api/factory/jobs');
    const waitingJobs = before.jobs.filter(job => job.status === 'waiting_manual');
    const targetJob = waitingJobs[0];
    const nextJob = waitingJobs[1];
    ...
    const projectionDiagnosis = {
      ok: waitingJobs.length === 2 && before.jobs.length === 2,
      source: 'BFF /api/factory/jobs',
      runtimeBuildId,
      waitingCount: waitingJobs.length,
      baselineRevision: worker.first.order.expectedWorkfileRevision,
    ...
    writeFileSync(join(evidence, 'pre-selection-state.json'), `${JSON.stringify(projectionDiagnosis, null, 2)}\n`);
    writeFileSync(join(evidence, 'projection-diagnosis.json'), `${JSON.stringify(projectionDiagnosis, null, 2)}\n`);
    if (!projectionDiagnosis.ok || !target.jobId || !target.candidateId || !nextTarget.jobId) {
      fail('task17_projection_diagnosis_failed');
    }
    process.stdout.write(`${JSON.stringify({ ok: true, projectionOnly: true, jobs: before.jobs.length, waiting: waitingJobs.length, evidence })}\n`);
  } finally {
