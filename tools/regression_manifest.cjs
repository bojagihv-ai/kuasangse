...
const BACKEND_SOURCES = [
  'backend/app.py',
  'backend/config.py',
  'backend/routes/api_draft_recovery.py',
...
const RUNTIME_SOURCES = [...new Set([
  ...
  'tests/frontend/optionsorter_runtime_apply_preservation.test.cjs',
  'tests/frontend/project_record_option_asset_restore.test.cjs',
  'tests/frontend/draft_recovery_document_scope.test.cjs',
  'tests/frontend/cafe24_weight_precision.test.cjs',
  'tests/frontend/native_model_cjk.browser.test.cjs',
  'tests/frontend/cafe24_option_image_links.test.cjs',
  'tests/frontend/task7_factory_cafe24_writer_retirement.test.cjs',
  'tests/frontend/control_tower_linked_style_tokens.test.cjs',
  'tools/verify_control_tower.cjs',
  'control_tower/tests/backend/test_health.py',
  'control_tower/frontend/control-tower.html',
  'control_tower/frontend/src/control-tower-client.mjs',
  'control_tower/frontend/src/bulk-intake.css',
  'control_tower/frontend/src/bulk-intake.mjs',
  'control_tower/frontend/src/product-intake.mjs',
  'control_tower/frontend/src/factory-operator-section-panel.mjs',
  'control_tower/frontend/src/factory-operator-source-panels.mjs',
  'control_tower/frontend/src/production-workbench.mjs',
  'control_tower/frontend/src/production-board.mjs',
  'src/factory-theme.css',
  'backend/tests/test_draft_recovery_limits.py',
...
function buildRegressionSteps(pythonExe) {
  ...
  return [
    ...
    nodeFile('CT-IMAGE-RETRY-01', '생산관제/이미지', '작업 자료 카드 재요청 중단·heartbeat DOM 보존', 'control_tower/tests/frontend/production-workbench-render.test.cjs', 'fast'),
    nodeFile('CT-INPUT-01', '생산관제/입력', '제품별 입력·Cafe24 분류·큐 추가 후 사진과 값 보존', 'control_tower/tests/frontend/intake-product-first.test.cjs', 'fast'),
    {
      ...nodeFile('CT-INPUT-02', '생산관제/입력', '선택 확인본·비동기 재확인·미해결 요청 신원과 입력 보존', 'control_tower/tests/frontend/bulk-selected-queue.test.cjs', 'fast'),
      args: ['--test', 'control_tower/tests/frontend/bulk-selected-queue.test.cjs', 'control_tower/tests/frontend/bulk-intake-automation-confirm.test.cjs'],
    },
    nodeFile('CT-REFERENCE-01', '생산관제/참조', '후보 판단·명시 입력 보존·비동기 작업 전환 차단', 'tests/frontend/factory_batch_reference_policy.test.cjs', 'fast'),
    ...
    nodeFile('CAFE24-WEIGHT-01', '7 전송', '포장 포함 무게의 kg 변환 정밀도와 실제 폼·전송 경로 보존', 'tests/frontend/cafe24_weight_precision.test.cjs', 'fast'),
    nodeFile('CAFE24-OPTION-IMAGE-01', '7 전송', '선택한 옵션 분리컷의 기존 상품 연결·승인 경계·원본과 품목 보존', 'tests/frontend/cafe24_option_image_links.test.cjs', 'fast'),
    ...
    nodeFile('CAFE24-REGISTRATION-02', '7 전송', 'HTML 이미지 오분류와 선택 원본·전송 보존', 'tests/frontend/cafe24_final_registration_write_regression.test.cjs', 'fast'),
    nodeFile('CAFE24-RECEIPT-02', '7 전송', '실제 등록 영수증의 이미지·재고·공통 치수 단위 대조', 'tests/frontend/cafe24_registration_receipt.test.cjs', 'fast'),
    ...
    nodeFile('NATIVE-QUEUE-01', '생산관제/작업 재개', 'A 저장 확인 중 다음 B는 ACK·heartbeat 대기 뒤 한 번 실행', 'tests/frontend/native_next_job_admission.test.cjs', 'fast'),
    nodeFile('TOWER-CSS-LINK-01', '생산관제/화면', '연결 스타일 토큰과 모듈 호출자의 캐시 번호·소스 일치', 'tests/frontend/control_tower_linked_style_tokens.test.cjs', 'fast'),
    { ...nodeFile('NATIVE-CJK-01', '생산관제/모델 안내', '375px 모델 안내·768px 작업큐 한글 단어 보존과 가로 넘침 방지', 'tests/frontend/native_model_cjk.browser.test.cjs', 'fast', 120000), browser: true },
    ...
    nodeFile('RESTORE-BATCH-01', '공통/작업파일', '배치 새로고침 시 작업파일 우선 복원·후보 ID 유지', 'tests/frontend/batch_worker_refresh_scope.test.cjs', 'fast'),
    nodeFile('RESTORE-BATCH-02', '공통/작업파일', '동일 원본의 빈 화면 복원·다른 제품 혼합 차단', 'tests/frontend/session_assets_same_work_factory_guard.test.cjs', 'fast'),
    ...
    nodeFile('SAVE-FALLBACK-01', '공통/데이터 격리', '축약 저장의 후보·선택·문서/탭 브랜치 보존', 'tests/frontend/critical_state_fallback.test.cjs', 'fast'),
    nodeFile('PROGRESS-COMPLETION-01', '공통/진행상태', '실시간 진행 및 검증된 등록 완료·새 승인 분리', 'tests/frontend/factory_goal_live_progress.test.cjs', 'fast'),
    ...
    nodeFile('PREFLIGHT-01', '공통/외부연동', '백엔드가 꺼지면 백엔드를 지목하고, 켜져 있어도 못 쓰면 그렇게 말한다', 'tests/frontend/preflight_blames_the_right_program.test.cjs', 'fast'),
    ...
    nodeFile('SAVE-42', '7 저장/내보내기', '필수값 즉시 저장·명시 화면 이동의 커밋 후 렌더', 'tests/frontend/force_save_immediate_persistence.test.cjs', 'fast'),
    ...
    nodeFile('FACTORY-PRICE-01', '조립공장/필수값', '대량 입력 판매가의 상세 섹션 전달과 명시적 0 보존', 'tests/frontend/factory_promotion_price_provenance.test.cjs', 'fast'),
    nodeFile('DETAIL-RESTORE-01', '조립공장/상세페이지', '선택컷 복원 배치와 현재 본문·이미지 재조립 보존', 'tests/frontend/factory_detail_terminal_trace.test.cjs', 'fast'),
    nodeFile('SAVE-IMAGE-REF-01', '공통/저장', '안정 이미지 주소 저장과 같은 작업 보관본 복원', 'tests/frontend/local_archive_section_restore.test.cjs', 'fast'),
    nodeFile('SAVE-REGISTRATION-RESTORE-01', '공통/저장', '같은 작업 등록 영수증·설정·publication 원자 복원', 'tests/frontend/factory_same_work_registration_restore.test.cjs', 'fast'),
    nodeFile('DRAFT-SCOPE-01', '공통/저장', '상품 복구본의 저장·목록·선택 복원 scope 일치', 'tests/frontend/draft_recovery_document_scope.test.cjs', 'fast'),
    nodeFile('SAVE-OPTION-RUNTIME-01', '공통/저장', '옵션 원본·분리컷 복원 시 바이트와 동일 작업 식별자 보존', 'tests/frontend/optionsorter_runtime_apply_preservation.test.cjs', 'fast'),
    nodeFile('SAVE-RECORD-OPTION-01', '공통/저장', '검증된 최근 작업파일의 옵션 원본·분리컷 복원과 작업 격리', 'tests/frontend/project_record_option_asset_restore.test.cjs', 'fast'),
    nodeFile('PREVIEW-ENTER-01', '2~5 이미지', '미리보기 버튼·자식 Enter의 native 활성화와 확정 단축키 분리', 'tests/frontend/factory_preview_enter_focus.test.cjs', 'fast'),
  ];
}
