...
async function main() {
  ...
  try {
    ...
    const activeBranchScope = seedResult.draftScope || '';
    ...
    const registeredRevision = reload.persisted.revisionRegistry?.[activeBranchScope] || {};
    ...
    const checks = [
      ...
      { ok: !!seedResult.projectId && seedResult.stateError === '' && seedResult.afterSave.appWorkspaceId === seedResult.projectId && seedResult.afterSave.stateFactoryWorkspaceId === seedResult.projectId && seedResult.afterSave.factoryWorkspaceId === seedResult.projectId && seedResult.afterSave.stateDbScope.startsWith(`${seedResult.projectId}::`) && seedResult.afterSave.stateCafeScope.startsWith(`${seedResult.projectId}::`) && seedResult.afterSave.dbScope.startsWith(`${seedResult.projectId}::`) && seedResult.afterSave.cafeScope.startsWith(`${seedResult.projectId}::`) && seedResult.afterSave.scope === activeBranchScope && seedResult.afterSave.authority?.scopeId === activeBranchScope && seedResult.afterSave.authority?.mode === 'offline-edit' && seedResult.afterSave.token?.workspaceId === seedResult.projectId && Number(seedResult.afterSave.token?.fence) > Number(beforeToken.fence) && seedResult.afterSave.buttons?.db?.disabled === false && seedResult.afterSave.buttons?.cafe24?.disabled === false, message: `saveCurrentProject 뒤 문서 identity 전환 또는 활성 branch 보존이 실패했습니다: ${JSON.stringify(seedResult)}` },
      { ok: seedResult.afterTransition.appWorkspaceId === seedResult.projectId && seedResult.afterTransition.stateFactoryWorkspaceId === seedResult.projectId && seedResult.afterTransition.factoryWorkspaceId === seedResult.projectId && seedResult.afterTransition.scope === activeBranchScope && seedResult.afterTransition.authority?.scopeId === activeBranchScope && seedResult.afterTransition.authority?.mode === 'offline-edit' && afterToken.workspaceId === seedResult.projectId && Number(afterToken.revision) >= 1 && Number(afterToken.fence) > Number(beforeToken.fence), message: `canonical document token 전환 또는 branch persistence scope 보존 불일치: ${JSON.stringify({ before: seedResult.before, afterSave: seedResult.afterSave, afterTransition: seedResult.afterTransition, flushQueuedPersistentState: seedResult.persistAfterTransition })}` },
      ...
      { ok: reload.appWorkspaceId === seedResult.projectId && reload.factoryWorkspaceId === seedResult.projectId && reload.scope === activeBranchScope && reload.operationToken?.version === 'factory-store:v1' && reload.operationToken?.workspaceId === seedResult.projectId && reload.operationTokenScope === projectScope && Number(reload.operationToken?.revision) >= 1 && Number(reload.operationToken?.fence) >= 1, message: `reload 후 문서 identity, 활성 branch 또는 operation token scope가 보존되지 않았습니다: ${JSON.stringify(reload)}` },
      ...
      { ok: reload.persisted.session?.projectId === seedResult.projectId && reload.persisted.session?.workspaceScope?.id === activeBranchScope && persistedRevision.scopeId === activeBranchScope && Number(persistedRevision.counter) >= 1 && typeof persistedRevision.writerId === 'string' && persistedRevision.writerId.length > 0 && JSON.stringify(persistedRevision) === JSON.stringify(registeredRevision) && reload.persisted.session?.dbScope === candidateProof.currentScopeKey && reload.persisted.session?.cafeScope === candidateProof.currentScopeKey, message: `persisted document/branch identity 또는 workspaceRevision이 reload 뒤 복원되지 않았습니다: ${JSON.stringify(reload.persisted)}` },
      { ok: reload.backend?.workspaceId === projectScope && reload.backend?.hasSnapshot === true && Number(reload.backend?.revision) === Number(backendAfterSave.revision) && reload.backend?.sha256 === backendAfterSave.sha256 && reload.backend?.bodyLength === backendAfterSave.bodyLength && reload.backend?.sha256 && reload.backend?.bodyLength > 0, message: `reload가 명시 저장된 project snapshot을 누락하거나 불필요하게 변경했습니다: ${JSON.stringify({ saved: backendAfterSave, reload: reload.backend })}` },
    ];
