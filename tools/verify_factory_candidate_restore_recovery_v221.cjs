...
function buildRestoreRecoveryChecks(proof) {
  ...
  return [
    ...
    { ok: proof.db[1]?.canApply === false && proof.cafe24[1]?.canApply === false && proof.buttons.dbForeign?.exists === false && proof.buttons.cafeForeign?.exists === false, message: `foreign 후보가 적용 불가인데도 버튼으로 렌더됐습니다: ${JSON.stringify(proof.buttons)}` },
