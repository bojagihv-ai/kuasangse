...
Cafe24 이미지 승인 용량 회귀: `task7_factory_cafe24_writer_retirement.test.cjs`는 상세15장을 한 변경안에 넣어 D1 2MB행 제한을 넘기던 실패를 재현한다. 전송본만 용량을 제한하고 한 장씩 기존 승인·실행·URL검증을 거친다. 원본·순서·전체15장, 작은 이미지 무변경, 용량 초과 시 전송 거절, JPEG 흰 배경과 기본 투명 리사이즈 보존을 검사한다. 대표 슬롯 합산 전송 예산도 동일 경계를 사용하며 실제 Cafe243027 반영은 별도 Chrome 수동 게이트다.

Native 저장 결과 회귀 `NATIVE-QUEUE-01`: 완료 영수증 뒤 동일 내용 자동저장으로 문서 번호만 증가한 경우에는 같은 작업·store·작업 토큰·전체 inputs/stages와 실제 서버 저장 증명을 모두 확인한 뒤 대기 잠금을 해제한다. 입력, 공정, store, 저장 증명 또는 검증 중 로컬 상태가 달라지면 계속 차단하고 추가 POST를 보내지 않는다. `native_next_job_admission.test.cjs`의 metadata-only 회귀는 기존 일일 검사 항목에서 실행하며 새 검사 서버나 포트를 추가하지 않는다.

생산관제 입력 회귀 `CT-INPUT-01`: 제품별 값은 CSV·공통값보다 우선하며, Cafe24 분류는 저장된 연결의 실제 목록에서 고른다. 큐에 추가된 제품의 사진·입력값·모델·정책은 새로고침 뒤에도 보존하고 다시 전송하지 않는다. `intake-product-first.test.cjs`를 일일 실행기에 포함한다. `intake-queue-flow.browser.test.cjs`는 발급된 `CONTROL_TOWER_QA_API_PORT`·`CONTROL_TOWER_QA_FRONTEND_PORT`에서 외부 쓰기 없는 브라우저 큐 추가·목록 이동을 확인한다. 실제 Chrome은 분류 선택·추가 전 확인·큐 보기·375/768/1280px 주 세로 스크롤·A 상태 보존을 별도 확인한다.

생산관제 입력 회귀 `CT-INPUT-02`: 기존 commonTargets는 제품 선택만 표시하고 공통값 적용·선택 큐 추가·준비된 전체 추가는 각각 명시 버튼으로 실행한다. 빈 선택은 전송0, 무효·기투입 제외 수와 확인 목록/전송 집합은 일치하며 stable group/입력/사진/모델 binding이 바뀌면 재확인한다. `bulk-selected-queue.test.cjs`는 실제 onClick callback의 async 경계와 기존 working-state round-trip을 일일 실행기에 연결한다. `intake-queue-flow.browser.test.cjs`의 격리 A/B/C fixture는 ready A 보존, B/C 선택, doubleclick 단일 실행, C 응답 유실 직후 reload→동일 key/요청 확인→중복 생성0을 증명한다. 기존 state/blobs 저장 확인 실패는 POST 전에 보류하며 새 retry 엔진/API를 만들지 않는다. 격리 fixture CSS는 올바른 MIME으로 원본 theme/bulk-intake.css를 serve하고 375/768/1280 도달성을 캡처한다. `bulk-intake-automation-confirm.test.cjs`는 실제 확인창 renderer와 정책 resolver를 Node에서 실행해 공통 기본과 제품별 실효 정책을 구분하고, 전체 자동 기본에서도 직접 선택 0/10/15항목 요약을 유지하며 상단에서 전체 AI를 단정하지 않는지 검증한다. 사용자 Native Chrome·운영12jobs 전후 증거는 별도 승인 QA이며 fixture로 대체하지 않는다.

생산관제 입력 회귀 `CT-INPUT-02` 추가 정답(2026-09-11): 시작 버튼은 `선택한 제품 추가` / `입력 완료 제품 모두 추가`로 표시하고 선택/추가 가능/전체 수는 기존 안내에서 구분한다. 기존 `intake-queue-flow.browser.test.cjs` 자동 회귀는 빈 선택과 선택2/가능0·1·2, 전체1·2·3, 확인·전송중·추가후 disabled, A 보존과 동일 C key 재시도를 유지한다. 375/768/1280px에서 같은 hint의 computed `wordBreak=keep-all`과 `정보는` DOM Range의 단일 줄을 함께 단언하며 주 스크롤·가로 넘침 검사를 유지한다. 증거는 `.omo/evidence/native-factory-batch-20260908/task-4/queue-labels-20260911/korean-wrap-final/`; 격리 통과를 실제 Chrome·새 전체daily/handoff로 대신하지 않는다.
...
| 공통 반응형 | `UI-CJK-01`, `UI-RESP-01` | 작은 창 한글 줄바꿈, 조립공장 탭 전체 라벨, 주 스크롤 도달성. `task8_responsive_cjk.test.cjs`에서 로컬 보관함 gallery와 카드 body의 최소 열 폭을 검증하고, 실제 이미지가 있는1280×640·960×640 화면에서 크게/가져오기/폴더 열기/경로 복사 버튼을 확인한다. | 음절 중간 분리·라벨 잘림·수평 overflow 0건. 문서 바깥 넘침뿐 아니라 카드 내부 버튼도 경계 안에 표시되고 줄바꿈되어야 한다. |
...
| `SAVE-CONCURRENCY-02` | 비동기 초안이 사용자 선택·선택 해제와 겹치는 저장 경쟁 | 같은 작업의 최신 사용자 변경은 유지하고, 늦게 끝난 작업의 독립 결과만 병합하며, 명시적 선택 해제는 되살리지 않음. 기존 컷 보관 정보 갱신과 새 컷·HTML 추가가 겹쳐도 새 자산 ID를 누락하지 않음. `factory_store_async_rebase.test.cjs` |
| `FACTORY-PRICE-01` | 대량 입력 판매가를 현재 이미지 분석 뒤 상세 섹션에 전달 | `salePrice=2000`은 프로모션·CTA 사실 요약에 `판매가: 2000 / 직접 입력`으로 전달. canonical 키 우선순위·명시적 0·DB fallback·입력 불변성 유지. `factory_promotion_price_provenance.test.cjs` |
...
| `OPT-14` | 옵션표 0장 실패 영수증 | 이번 실행의 시트 생성이 모두 실패하면 첫 실제 시트 오류를 생성 영수증과 `options` 단계 `blocked` 상태에 그대로 표시한다. 이전 옵션표는 새 실행 성공으로 세지 않으며, 명시적 색상이미지 안씀과 `text_only` 결과는 유지한다. `factory_option_zero_output.test.cjs` |
...
| `NATIVE-QUEUE-01` | Native 수동 재개 뒤 다음 작업 admission 경합 | A의 정상 resume가 terminal이어도 native 저장 readback이 끝나기 전 B가 준비될 수 있다. B는 기존 validate/ACK·heartbeat를 유지하며 검증된 A clean까지 대기하고 원본 진입·complete는 각 1회, fail은 0회여야 한다. B가 A jobs 조회 전/후 준비되는 두 순서와 unknown/lost/stale 저장 확인·반복 polling을 검증한다. 초기부터 B queued/running이면 A preflight는 여전히 차단한다. 일반/hidden worker·잘못된 order·실제 실패 처리는 유지하며 ACK된 대기를 queued로 표시하지 않는다. 새 탭에서 같은 제품 신원이 복원돼도 runtime workflow가 비었으면 저장본 열기가 기존 restoreOnly를 한 번 수행하고 원본 체크포인트 저장까지 성공해야 한다. 이미 열린 유효 체크포인트는 불필요하게 다시 복원하지 않으며 입력·사진·선택·로그는 보존한다. 다른 제품의 waiting_manual/blocked/completed에 남은 순수 autoResumePending은 현재 제품 재개를 막거나 다른 제품을 변경하지 않는다. 현재 제품의 autoResumePending과 모든 pendingSelection/dispatched/queued/running 및 알 수 없는 상태의 대기는 기존대로 차단한다. 저장 영수증 이후 동일 내용 자동저장으로 문서 번호만 앞서도 동일 신원·storeRevision·inputs·stages 및 저장 증명을 모두 확인하고 승인된 큐 체크포인트 번호로 한 번만 재개한다. 입력·공정·store·큐 체크포인트가 실제 변경되면 전송 없이 차단하고 새 로컬값을 보존한다. 체크포인트 저장 증명은 commit 전 로컬 project 사본이 아닌 기존 server restore의 저장 snapshot+권위 revision을 사용하고 오래된 값·외부 신원·저장본 없음은 거부한다. 비신규 재개의 옵션 여부가 pending이면 투입값 provided/none을 적용하되 이미 명시된 옵션 선택은 덮어쓰지 않는다. document 캡처의 경쟁사 보조 클릭도 탭 contract.invoke를 우선 사용하고 Native pending 오류 뒤 직접 실행으로 우회하지 않으며 탭이 없는 legacy 경로는 유지한다. competitors 소유 명령이 확정한 선택과 명시적 해제는 저장 예약 전에 기존 직렬화 사본에 분리 복사돼야 하며 다른 작업 토큰은 이를 변경할 수 없다. 복원 시 검색어가 제품명과 달라도 factoryProductKey/productKey/workProductName의 기존 소유 정체성이 같으면 후보·선택·상세를 보존하고 실제 다른 상품 정체성은 계속 분리한다. 분석 전 후보만 담긴 저장본은 검증된 대상 작업의 제품·run·사진 지문이 모두 일치할 때 후보 partition만 병합하며 분석 스코프가 없는 외부 분석·플랜은 가져오지 않는다. `native_next_job_admission.test.cjs`, 인접 `native_manual_reference_resume.test.cjs`/`native_factory_batch_boot.test.cjs`. 실제 Chrome·서버 큐 상태는 별도 수동 게이트이며 fixture 결과로 대체하지 않는다. |
| `NATIVE-CJK-01` | Native 모델 안내 CJK 줄바꿈·긴 ID overflow·조건부 버튼 숨김 | 375px headless Chromium의 실제 `createNativeBatchConsoleView` export에서 측정한 한글 glyph 경계에 `사용` Range를 놓고 한 줄 rect 1개를 요구한다. 공백·하이픈 없는 긴 모델 ID는 자연 컨테이너 폭에서 document 가로 overflow가 없어야 한다. 별도 viewport 연동 fixture에 실제 `src/factory-theme.css`를 적용한 headless Chrome에서 `nativeBatchReconcile`·`nativeBatchLocalRecovery`의 기본 숨김→표시→재숨김과 375/768/1280px·높이 280px의 기존 헤더 행동 도달성·가로 넘침을 검증한다. 조건부 상태는 fixture의 `hidden` 속성으로 제어하며 controller workflow 검증을 주장하지 않는다. 이 격리 fixture는 실제 Chrome 수동 게이트를 대체하지 않는다. `native_model_cjk.browser.test.cjs` |
...
| `ANALYSIS-LLM-01` | 모델 설정·경쟁사 분석 모델 선택 | 모델 선택 저장·F5 복원·분석 LLM 재렌더 선택 유지와 함께, 원본 프롬프트 함수 실행 baseline/RED→GREEN으로 JSON 전체 키·10개 기준·모든 실제 섹션·스타일·정규좌표 근거, 모델·전처리·240초 호출 설정을 보존한다. 첨부 픽셀 전용·외부 도구 금지·이미지 내 지시 데이터 처리, 서술 1~2문장·기준별 문제 최대 2개/개선조치 2개 정도·중복 금지·불확실/빈값/null 근거·새 상품사실 금지를 검사하며 고정 총량으로 항목을 누락하지 않는다. 프롬프트 계약 통과는 실모델 시간 초과 해결 증거가 아니다. `competitor_analysis_model_selection.test.cjs` |
...
| `SAVE-IMAGE-REF-01` | 경량 저장의 안정 이미지 주소 보존 | `stripSectionImages`는 HTTP/HTTPS와 상대 local-archive 주소를 유지하고 `data:`/`blob:`만 복원 마커로 남긴다. null·빈 값·기존 마커와 동일 작업 복원 경계를 함께 검증한다. `local_archive_section_restore.test.cjs`; 실제 Chrome 저장·재열기 뒤 이미지 주소와 표시 수를 비교한다. |
| `DETAIL-RESTORE-01` | 선택컷 배치와 이미지 복원의 현재 문구 보존 | EF 주배치 저장본을 복원한 뒤 D50/EF를 재조립해도 각 선택컷은 한 번씩 남는다. 정확한 미편집 자동 중복만 정리하고 사용자 이동·캡션·metadata는 보존한다. 이미지 마커를 복구할 때 현재 2,000원 문구를 보관본 0원 문구로 덮지 않으며, 복구되지 않은 이미지를 완료로 세거나 유료 전체 재생성으로 넘기지 않는다. `factory_detail_terminal_trace.test.cjs`; 선택 A컷의 기존·부분 HTML 우선순위는 기존 `CAFE24-REGISTRATION-02`도 함께 실행한다. 실제 Chrome 복원·재조립 뒤 본문·선택·이미지 수를 read-back하기 전에는 완료가 아니다. |
...
| `SAVE-11B` | 상세 문서와 섹션 이미지 구분 | HTML 보관본을 `/image`로 변환하지 않는다. 선택한 섹션 변형의 보관 원본은 현재 이미지가 잘못 덮인 경우에도 복원하며, 숨긴 섹션의 원본을 삭제하지 않고 전송 목록에서만 제외한다. `cafe24_final_registration_write_regression.test.cjs` |
...
| `SAVE-24` | 최근 작업파일 표시·새 작업 뒤 파일 위치 힌트 | 저장한 `.kuasangse` 파일명은 제품명과 분리해 최근 카드에 표시하고, 새 작업은 현재 파일 표시를 비우되 같은 세션의 저장 핸들은 다음 불러오기 `startIn`으로 유지. 긴 상단 파일명·작업ID·로컬 보관 경로는 작은 창에서도 폭 안에 줄바꿈하고 확장자 자체는 나누지 않는다. `menu_navigation_characterization.test.cjs`, `verify_factory_new_file_blank_cdp_v85.cjs`, `verify_workfile_picker_location_cdp_v121.cjs` |
...
| `SAVE-42` | 필수값 즉시 저장·명시 이동의 커밋 후 렌더 | `factory/fields:commitField` 등 `forceSave:true` 명령은 최신 canonical factory snapshot을 즉시 `saveLastWorkNow()`에 넘긴다. 이후 탭 전이나 렌더가 다른 저장 타이머를 교체해도 확정 필수값이 session commit 경계에 도달해야 한다. 경쟁사 메뉴/리포트 명시 이동은 성공한 현재 커밋 뒤 한 번 렌더하며 거절·실패·stale·이미 떠난 화면을 강제 전환하지 않는다. `force_save_immediate_persistence.test.cjs`, 실제 Chrome 수동 게이트 |
...
| `SAVE-RESTORE-01` | 복원 전 소스 실패와 저장본 없음의 구분 | `workspace-persistence-orchestrator` 의 `restore` 는 소스별 실패를 삼키고 다음 소스로 넘어가되, 시도한 모든 소스가 예외로 끝나면 `onDiagnostics({kind:'restore-all-sources-failed'})` 로 알린다. 반환값과 제어 흐름은 그대로여서 기존 호출자에 영향이 없다. 예외 없이 데이터만 없는 정상 상태와 일부 소스만 실패한 경우는 알리지 않는다. 브라우저 팩토리는 이를 기존 저하 보고 채널로 넘겨 사용자가 '작업이 없음'과 '불러오지 못함'을 구분할 수 있게 한다. `restore_all_sources_failed_signal.test.cjs` |
| `SAVE-FENCE-01` | archive 쓰기 후 권한 검사 범위 | `archive-adapter` 의 쓰기 후 검사는 **스코프 변경만** 막는다. 그 시점의 쓰기는 서버가 `leaseId`·`fencingToken`·`expectedRevision` 으로 이미 CAS 검증해 반영한 뒤라, 같은 스코프 안에서 lease 가 갱신·재획득된 것만으로 실패 처리하면 되돌려지지도 않는 쓰기를 오보하는 셈이다. (하이드레이션 끝의 draft 브랜치 권한 복구가 왕복 중 토큰을 바꿔 2026-08-22 GENERATE-01 이 7회 중 2회 실패했다.) 스코프가 바뀐 경우는 결과를 다른 작업에 적용하면 오염되므로 계속 `STALE_FENCE` 로 막는다. `archive_lease_renewal_not_stale.test.cjs`, `archive_offline_branch_authority.test.cjs` |
| `LLM-FALLBACK-02` | Claude 구독 로그인 폴백과 능력 인지 체인 | API Hub 의 `claude-subscription-oauth` 브리지(`/api/claude-oauth/*`)를 폴백 1순위로 쓴다. `ClaudeOAuthAPI` 는 `GptOAuthAPI` 를 상속해 전송 계층만 바꾸므로 대형 분석 프롬프트를 중복 작성하지 않고, 브리지가 명시한 successContract(`usedClaudeOAuth`·`rawTokenReturned`)와 passCondition(`claudeLoginReady`·`mode`·`usesApiKey`)을 그대로 검증한다. Anthropic API 키 경로(`anthropic.api_key`, `ANTHROPIC_API_KEY`, `api.anthropic.com`)는 사용 금지다. 브리지는 프롬프트 텍스트만 받으므로 이미지 판독·생성을 못 한다. 따라서 폴백은 단일 대상이 아니라 `providerSupportsLlmMethod` 기반 체인이어야 하며, `analyzeImage`·`analyzeCompetitorImages`·`generateImage` 요청에서는 Claude 를 건너뛰고 vision 이 되는 다음 후보(OpenAI / 로컬 Ollama)로 넘어간다. 기본 실행 provider 와 동일한 provider·모델로는 재시도하지 않되 체인은 계속 이어간다. 능력 있는 후보가 하나도 없으면 폴백하지 않는다. 모델 설정은 브리지 uiRequirements 대로 모델 4개·노력 5단계·상태 새로고침/로그인/강제 재로그인 버튼을 모두 노출한다. `claude_oauth_fallback.test.cjs` |
...
| `CMD-UI-02` | 조립공장 명령·DB 재검색·옵션 분류기 경계 | DB 재검색/추가검색은 검색어만 반영하고 제품 정체성·필수값·사진·색상옵션을 보존한다. 실제 검색은 저장한 검색어를 사용한다. `openFactoryOptionSorter`는 소유 중인 factory draft로 저장하며 `factory/assets` 명령에 `lastSavedAt` 메타데이터 쓰기를 섞지 않는다. |
...
| `RESTORE-BATCH-01` | 배치 작업자 새로고침 | 연결된 배치 작업파일의 서버 사본으로 부족한 자료를 보완한다. 자동 보관함 복원으로 후보·선택 ID를 새로 발급하지 않는다. 빈 워커는 다른 제품을 불러오지 않는다. `batch_worker_refresh_scope.test.cjs`, 실제 Chrome 새로고침 전후 exact set 비교 |
| `RESTORE-BATCH-02` | 빈 화면·이미지 우선 복원 | 아직 적용하지 않은 저장본의 사진을 현재 작업으로 계산해 복원을 차단하지 않는다. 이미지가 먼저 복원되면 동일 작업 범위·원본 지문이 일치할 때만 입력·옵션·선택을 이어받고, 다른 제품명·다른 원본은 거부한다. `session_assets_same_work_factory_guard.test.cjs` |
...
| `SAVE-49` | 같은 작업 경쟁사 분석 중복 없는 복원·저장 | `analysisResult.sections_found`와 `analysisResult.page_score.criteria`의 정확한 객체 중복만 키 삽입 순서와 무관하게 정리한다. 실제 preserve→apply→build 연결 경로와 서버 keep→drop→POST/GET에서 섹션16·기준10 및 전체 분석 JSON이 유지되어야 한다. 고유 행·내용·행/배열 순서·타입은 보존하고 다른 배열·선택 ID·사진·필수값 감소 보호 및 빈 분석 복구는 유지한다. `competitor_analysis_persistence.test.cjs`, `test_analysis_invalidation_marker.py`, `test_workspace_authority.py` |
| `SAVE-REGISTRATION-RESTORE-01` | 같은 작업 등록 기록·설정·publication 복원 | 오래된 실패와 최신 검증, 최신 실패·pending과 오래된 검증, 동일 `capturedAt`의 후속 검증, 비어 있거나 `null`인 thin snapshot, 실제 normalizer 선행 경로와 등록 시각 0인 absent/null receipt, 비교 누락·불일치와 receipt identity 충돌, 다른 job/run/input/Cafe24 대상, 변경·미변경 `UserTouched` 설정, receipt 없는 publication-only 현재 상태를 포함한 10개 시나리오에서 receipt를 필드 혼합 없이 선택하고 A의 F/F/100·자산·수동값을 낮추지 않는다. `factory_same_work_registration_restore.test.cjs`. 실제 Chrome 복원 read-back과 전체 `verify:daily`/`verify:handoff`는 별도 게이트다. |
| `PREVIEW-ENTER-01` | 미리보기 버튼 Enter 포커스 보존 | Next/Prev/Close/전송/Confirm 버튼과 자식의 Enter는 document 확정 단축키가 취소하지 않고 기존 click 경로를 따른다. bare Enter 확정·Confirm 1회·방향키·Escape·mouse·previewOnly는 유지한다. `factory_preview_enter_focus.test.cjs`. 배포 및 실제 Chrome 화면/A read-back은 별도 인계 게이트다. |
...
- `CAFE24-WEIGHT-01`: 사용자가 확정한 포장 포함 무게 15g는 실제 `product_weight` 전송값에서 0.015kg로 보존한다. 공통 단위 변환, 일반·선택형 폼, 직접 매핑·사용자 정의 필드의 최종 update 경로와 입력 불변을 `cafe24_weight_precision.test.cjs`로 검증한다. API의 미확인 소수 정밀도를 이유로 임의 반올림하거나 최소 중량을 만들지 않는다. 실제 Cafe24 수락값과 재조회 결과는 승인된 상품의 실환경 게이트에서 별도로 확인한다.
- `CAFE24-OPTION-IMAGE-01`: 명시적으로 선택한 옵션 결과의 분리컷만 기존 승인 이미지 업로드 경로와 옵션 PUT에 연결한다. 같은 이름의 다른 결과 제외, 원본 바이트·기존 옵션/이미지/품목 보존, 이미지 변경만 있을 때 품목·재고 쓰기 0, 승인 실패/경로 누락/선택 변경 후 추가 쓰기 차단, 부분 업로드 재사용과 실제 최종 등록 함수의 전달을 `cafe24_option_image_links.test.cjs`로 검증한다. 원격 echo는 옵션명뿐 아니라 두 이미지 경로와 기존 구조도 대조한다.
- `TOWER-CSS-LINK-01`: `verify:control-tower`의 토큰 검사는 HTML이 실제로 연결한 공유 테마·대량입력 CSS까지 포함한다. 연결이 빠진 파일의 정의를 임의로 더하지 않으며 링크 삭제·토큰 정의 삭제·미정의 토큰 추가는 계속 실패해야 한다. 모듈 캐시 검사도 마지막 호출자만 확인하지 않고 동일 모듈의 모든 호출자 번호가 같은지, 내용 변경 뒤 기존 번호를 재사용했는지 검사한다. `control_tower_linked_style_tokens.test.cjs`의 메모리 검증으로 확인하고 새 화면의 실제 스타일·스크롤 확인은 Chrome 게이트에 남긴다.
- 이번 생산관제 실환경 등록 게이트는 사용자가 승인한 기존 Cafe24 상품 **3027 갱신만** 허용한다. 위 일반 신규 등록 예제로 새 상품을 만들지 않는다. 실제 Chrome에서 최종 등록을 실행하기 전후에 3027의 가격·분류·옵션·품목·상품 이미지·상세 HTML·포장 포함 0.015kg를 다시 읽고, 선택한 두 옵션 이미지의 응답 경로와 화면 결과를 확인한다. 자동 테스트는 이 게이트를 대신하지 않는다.
- `NATIVE-CJK-01` 작업큐 보강: 768px의 좁은 상태 열에서도 `필요`의 두 글자가 서로 다른 줄로 나뉘지 않아야 한다. 기존 원본 view 모듈을 렌더한 Chromium에서 실제 Range 좌표로 확인하고, 기존 375px 모델 안내·긴 식별자·조건부 버튼·주 스크롤 검사는 유지한다. 사용자 Chrome의 375/768/1280px 사후 화면도 별도로 확인한다.

- `CT-REFERENCE-01`: 직접 선택 정책은 후보만 수집한다. AI 판단은 실제 입력/후보 이미지와 허용 식별자만 사용하며 불확실·충돌 시 보류한다. 비동기 후보 적용·저장 중 작업 전환, 같은 작업의 run 변경, store revision 변경은 이전 판단의 적용과 체크포인트 쓰기를 차단한다. 사전 입력값·새 수동값·빈 수동 초안은 자동 원본값으로 덮지 않는다. `factory_batch_reference_policy.test.cjs`, `factory_batch_required_input_preservation.test.cjs`; 실제 생산관제에서 DB/필수값/경쟁사 대기→다음 제품→원본 확인 후 이어하기를 별도 확인한다.

- `CT-IMAGE-RETRY-01`: 화면에서 제거된 작업 자료 카드는 이미지 재요청을 하지 않는다. 동일 입력·출력 자료의 heartbeat는 기존 카드와 실패 안내를 유지하고, 연결된 카드의 유한 fallback과 다른 작업 전환은 보존한다. 실제 Chrome에서 같은 이미지 노드 연결 유지와 일정 구간 `thumbnail?retry=` 요청 수 증가 0을 확인한다. `control_tower/tests/frontend/production-workbench-render.test.cjs`

- `SAVE-FALLBACK-01`: minimal/emergency 저장에서도 `workspaceBranch`와 `workspaceKind`를 보존한다. 실제 identity validator로 같은 문서/탭 복원을 허용하고 다른 문서를 거절한다. 후보·DB/VM 선택은 그대로여야 한다. 실제 Chrome 새로고침에서 `WORK_IDENTITY_SCOPE_CONFLICT`가 재발하지 않아야 한다.
- `PROGRESS-COMPLETION-01`: `export:done`은 기존 검증 경계가 인정한 등록 영수증이 있을 때만 완료로 투영한다. 중간 컷 완료·등록 검증 불일치는 전체 완료로 올리지 않고, `error`는 실패로 표시한다. 과거 등록 이력은 새 Cafe24 쓰기를 승인하지 않는다.
- `SAVE-CONCURRENCY-01` 보강: A 이미지 보관 쓰기보다 늦게 요청된 B 편집권 획득은 A를 추월하지 않는다. 같은 스코프 재획득·대기 반납·stale A 쓰기 거절을 함께 확인하며, 실제 생산관제 복원 후 저장 경고 및 입력/후보/선택 무감소를 읽는다.
- `UNIT-FE-01` 보강: 이미지 저장 실패 경고가 남아 있으면 동일 fingerprint도 실제 IndexedDB 저장을 재시도한다. 성공 뒤 이미지 재읽기와 경고 해제를 확인하고, 주입한 쓰기 실패에서는 이전 이미지와 경고가 유지돼야 한다.
- `SAVE-CONCURRENCY-01` 인접 adapter 계약 보강: 새 초안의 bootstrap 문서 ID는 비어 있어야 한다. 명시적 저장 문서와 `project:` legacy fallback은 보존한다. `SAVE-25` 및 `SAVE-REFUSE-01`의 새 작업/새로고침 시나리오와 함께 확인한다.
- 생산관제 `production-board.test.cjs`: 완료품의 제품명·공급가는 기존 저장값에서 표시하되 직접 지정된 필수값을 우선한다. 저장된 Cafe24 값의 배지가 맞고 단순 열람은 입력 수정이나 외부 등록을 실행하지 않아야 한다.
...
### 2026-09-14 옵션 원본 복원 회귀 SAVE-OPTION-RUNTIME-01

실제 생산관제의 같은 작업 재개 후 옵션 원본2장이 복원되지 않는 실패를 기준으로 `optionsorter_runtime_apply_preservation.test.cjs`를 매일 회귀에 추가한다. 원본 런타임 적용은 무참조 원본·분리컷의 유일한 바이트를 보존하고, 경량 사본은 큐 입력 재매칭에 필요한 지문과 색상 metadata를 보존해야 한다. 최근24개 경계 밖의 무참조 이미지, 입력불변, 다른작업 거부 및 같은 작업의15g·로그·섹션·선택 불변을 확인한다. 이 자동검증은 실제 Chrome 복원·이미지 표시·저장/재개/Ctrl+F5 read-back의 대체가 아니다.

복구 사본 용량 경계는 `backend/tests/test_draft_recovery_limits.py`로 `UNIT-BE-01`에 포함한다. 원본A처럼24MiB를 넘는 이미지 포함 snapshot은 기존 `Config.MAX_CONTENT_LENGTH` 한도 안에서 저장·목록·원문 재조회가 같아야 한다. 설정한 한도를 넘는 복구본은413, 잘못된scope/객체는400, 기존최신5개보존은유지한다. 운영원본backend 재기동과 실제Chrome POST/read-back은 별도게이트다.

상품 복구본 scope 회귀 `DRAFT-SCOPE-01`: 저장 경로와 기존 복구 버튼의 목록/선택 복원은 현재 document scope를 우선하고 미저장 작업만 draft scope를 사용한다. 저장된 상품에서 이미지 원본 복원 경고가 있으면 기존 `이미지 원본 복구본 보기` 동작을 노출한다. 정상 저장 프로젝트·무관 경고는 기존 숨김을 유지하고, 다른 상품 응답은 identity 재라벨링 없이 기존 scope 가드가 거절해야 한다. `draft_recovery_document_scope.test.cjs`와 현재 로그 보존 회귀를 함께 확인하며 실제 Chrome의 같은 상품 복구 read-back은 별도필수다.

### 2026-09-14 최근 작업파일 옵션 복원 SAVE-RECORD-OPTION-01

`project_record_option_asset_restore.test.cjs`는 신원이 검증된 같은 프로젝트의 최근 작업파일을 명시적으로 여는 경계에서, 최상위 옵션 목록이 비어 있어도 `assetPayload`의 원본·옵션 결과가 실제 런타임에 복원되는지 확인한다. 일반 background/session 초기화 방어, 다른 작업·입력 지문 거부, 기존 revision 검증과 같은 작업 A 보존은 유지한다. 새 탭 저장본 열기→옵션 원본2장/결과3개·분리컷6개→저장/재개/재복원 실환경 검증을 별도로 통과해야 한다.

동일 회귀의 원본 검증 순서 보강: `validateWorkspaceSnapshotIdentity`·실제 resolver/rebind·실제 revision 비교 모듈로 프로젝트 범위와 revision 없는 draft 범위를 실행한다. 명시적 저장본의 원본 식별자·버전 검증은 같은 작업 fallback 병합과 편집 브랜치 재결합 전에 끝내야 한다. 잘못된 원본은 기존 롤백 경로로 차단하고, 이전 상태가 누락 검증 정보를 보충해서 통과시키면 실패다. 원본2/결과3/분할6·20자산·107로그 순서와 입력불변, 잘못된 원본 적용 전 A 불변을 함께 확인한다.

공유 이미지 참조 복원 보강: 경량 프로젝트/저장 버전의 옵션 원본과 분리 이미지 데이터는 기존 document sessionAssets에서 동일 작업 identity·scope·유효한 최신 revision을 확인한 뒤 유일한 이미지/결과 ID와 분리컷 sourceId·이름·순서·파일명으로 연결한다. 다른 작업·입력·버전, 중복 ID, 색상명 변경은 연결하지 않으며 기존 inline 데이터·선택·로그는 보존한다. 실제 `loadSnapshotRecord` 성공과 저장 실패/신원 불일치 rollback을 실행하고, 실제 저장본 metadata와 불변 A의 원본2/결과3/분리6 byte hash를 별도 비교한다. 실제 Chrome 저장본 열기·저장/재개·Ctrl+F5는 여전히 별도 필수 게이트다.

### 2026-09-14 원본 백업 참조 및 저장 완료 로그 보존
기존 SAVE-RECORD-OPTION-01은 실제 applyProductImageBackupPayload로 최상위 전체 백업/중첩 경량 참조의 복원, 잠긴 지문 불일치 차단, 다음 저장 백업, 누락 참조 0을 검사한다. 문서 적용 전 기존 서버 복원 API의 권위 리비전과 작업/실행/상품/이미지 신원을 확인하고 기존 중앙 보존 가드로 일반 로그를 합친다. 최상위·중첩 로그, 순서·반복 multiplicity, 저장 버전 실제 로더, 서버 읽기 실패 시 A 보존을 확인한다. Chrome v1509 실제 저장 A 보존 실패는 자동 통과로 대체하지 않으며 새 빌드 저장/재개/새로고침 재검증이 필요하다.

### 2026-09-14 원본 백업 출처 보존
`project_record_option_asset_restore.test.cjs`의 실제 공통 백업 함수 검사에 canonical factory 원본/primary 출처 보존을 추가했다. 최근 작업을 열어 legacy `state.factory`가 비어 있어도 정식 store에 복원된 원본을 저장 백업에 포함해야 한다. 같은 입력 지문만 허용하는 기존 외부 이미지 거부 검사는 유지한다. 실제 Chrome 저장·재개·새로고침과 백업 출처 네 항목 read-back은 별도 게이트다.
