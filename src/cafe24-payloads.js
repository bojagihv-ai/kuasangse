...
/**
 * 상품 추가정보는 우리가 key 와 value 만 보내는데, Cafe24 는 거기에 name(표시 이름)을
 * 덧붙여 돌려준다. 통째로 비교하면 값이 같은데도 다르다고 나온다. 실측 2026-08-25:
 * 보낸 [{key:'custom_option1', value:'가로4cm*세로20cm'}] 가 저장된
 * [{key:'custom_option1', name:'사이즈', value:'가로4cm*세로20cm'}] 와 다르다고 잡혔다.
 * 우리가 정한 것은 key 와 value 뿐이니 그 둘만 본다.
 */
function factoryCafe24AdditionalInfoPairs(value) {
  if (!Array.isArray(value)) return value;
  return value
    .map(row => (row && typeof row === 'object'
      ? { key: String(row.key ?? '').trim(), value: String(row.value ?? '').trim() }
      : row))
    .filter(row => !row || typeof row !== 'object' || row.key || row.value);
}
...
function factoryCafe24ProductFieldValuesEqual(field, expected, actual) {
  ...
  if (field === 'additional_information') {
    return factoryCafe24ValuesRoughlyEqual(
      factoryCafe24AdditionalInfoPairs(expected),
      factoryCafe24AdditionalInfoPairs(actual),
    );
  }
  return factoryCafe24ValuesRoughlyEqual(expected, actual);
}

// Cafe24 상품 조회가 애초에 돌려주지 않는 필드들. 보냈다고 해서 되읽을 수 있는 것이 아니다.
// 이것들을 "빠졌다/다르다" 로 세면, 멀쩡히 올라간 상품이 등록 실패로 기록된다.
// 실측 2026-08-25: #3022 는 스토어에 9색 옵션과 재고 99 가 정상인데, 조회 응답에 없는
// options / use_additional_option / use_attached_file_option 때문에 불일치로 남았다.
// 이 영역은 등록 후 대조가 옵션값·품목 수·옵션별 재고로 따로 확인한다.
const FACTORY_CAFE24_ECHO_UNREADABLE_FIELDS = Object.freeze(new Set([
  'options',
...
function factoryVerifyCafe24ProductEcho(product = {}, raw = {}) {
  const fields = Object.keys(product || {})
    .filter(key => !FACTORY_CAFE24_ECHO_UNREADABLE_FIELDS.has(key));
  const missing = [];
  const mismatches = [];
  fields.forEach(key => {
...
function factoryCafe24DetailForeignProductCheck(html = '', factory = factoryRuntimeReadFactory(), appState = {}) {
  ...
  const locations = conflicts.length
    ...
    : [];
  // 잠근 섹션은 「섹션 다시 만들기」가 일부러 보존한다. 그래서 잠긴 채로 오염돼 있으면
  // 몇 번을 다시 만들어도 그대로다 — 실측 2026-08-29: specifications 가 잠겨 있어
  // 재생성 네 번이 모두 헛돌았고, 화면에는 잠금 때문이라는 단서가 없었다.
  const sectionLocks = appState && typeof appState.sectionLocks === 'object' && appState.sectionLocks
    ? appState.sectionLocks
    : {};
  const lockedLocations = locations.filter(key => sectionLocks[key]);
  return { ok: conflicts.length === 0, conflicts, expectedTerms, locations, lockedLocations };
}
      ...
      if (html && /<[^>]+>/.test(String(html))) {
        ...
        if (!foreignCheck.ok) {
          return {
            ...
            blocked: true,
            sectionCount,
            message: `상세페이지 HTML에 현재 상품과 다른 상품명 단서가 남아 있어 전송하지 않습니다: ${foreignCheck.conflicts.slice(0, 3).join(', ')}${(foreignCheck.locations || []).length ? ` (남은 섹션: ${foreignCheck.locations.join(', ')})` : ''}. ${(foreignCheck.lockedLocations || []).length ? `${foreignCheck.lockedLocations.join(', ')} 은(는) 잠겨 있어 다시 만들어도 그대로입니다. 잠금을 먼저 풀어 주세요.` : '현재 상품 기준으로 섹션을 다시 생성해주세요.'}`,
          };
        }
