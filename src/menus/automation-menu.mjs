...
function one(root, selector) { return root?.querySelector?.(selector) || null; }

export function createAutomationMenu(capabilities = {}) {
  const getSnapshot = fn(capabilities, 'getSnapshot');
  const assertMutable = fn(capabilities, 'assertMutable');
  const updateAutomation = fn(capabilities, 'updateAutomation');
  const persistAutomation = fn(capabilities, 'persistAutomation');
  const refreshServer = fn(capabilities, 'refreshServer');
  const applyServerSnapshot = fn(capabilities, 'applyServerSnapshot');
  const pollServerStatus = fn(capabilities, 'pollServerStatus');
  const applyServerStatus = fn(capabilities, 'applyServerStatus');
  const connectDrive = fn(capabilities, 'connectDrive');
  const setFolder = fn(capabilities, 'setFolder');
  const saveProfile = fn(capabilities, 'saveProfile');
  const runMode = fn(capabilities, 'runMode');
  const applyRunResult = fn(capabilities, 'applyRunResult');
  const setModeEnabled = fn(capabilities, 'setModeEnabled');
  const refreshOutput = fn(capabilities, 'refreshOutput');
  const applyOutputResult = fn(capabilities, 'applyOutputResult');
  const openOutputPreview = fn(capabilities, 'openOutputPreview');
  const startAutomation = fn(capabilities, 'startAutomation');
  const stopAutomation = fn(capabilities, 'stopAutomation');
  const runOnce = fn(capabilities, 'runOnce');
  const resetProcessed = fn(capabilities, 'resetProcessed');
  const requestRender = fn(capabilities, 'requestRender');
  const getOperationToken = fn(capabilities, 'getOperationToken');
  const setIntervalFn = fn(capabilities, 'setIntervalFn');
  const clearIntervalFn = fn(capabilities, 'clearIntervalFn');
  const formatServerDriveMessage = fn(capabilities, 'formatServerDriveMessage');
  const renderModeEditor = fn(capabilities, 'renderModeEditor');
  const renderPlacementPanel = fn(capabilities, 'renderPlacementPanel');
  const makeDefaultProfile = fn(capabilities, 'makeDefaultProfile');
  const disabledAttr = fn(capabilities, 'disabledAttr');
  const escapeHtml = fn(capabilities, 'escapeHtml');
  const escapeAttr = fn(capabilities, 'escapeAttr');
  const extractFolderId = typeof capabilities.extractFolderId === 'function' ? capabilities.extractFolderId : value => String(value || '').trim();
  const parsePositiveInt = typeof capabilities.parsePositiveInt === 'function' ? capabilities.parsePositiveInt : (value, fallback, min, max) => Math.max(min, Math.min(max, Number.parseInt(value, 10) || fallback));
  const getDraftCuts = typeof capabilities.getDraftCuts === 'function' ? capabilities.getDraftCuts : () => [];
  const updateDraftCut = typeof capabilities.updateDraftCut === 'function' ? capabilities.updateDraftCut : () => {};
  const recoverDraftCuts = typeof capabilities.recoverDraftCuts === 'function' ? capabilities.recoverDraftCuts : () => {};
  const reportError = typeof capabilities.reportError === 'function' ? capabilities.reportError : () => {};
  ...
  const current = operation => active && operation.generation === generation && operation.token === getOperationToken();
  ...
  function mutable() { assertMutable(); }
  ...
  const commands = {
    ...
    connectDrive: { capability: 'automation:write', execute(clientId) { mutable(); return guarded(connectDrive, null, clientId); }},
    ...
    saveProfile: { capability: 'automation:write', execute(value) { mutable(); return guarded(saveProfile, applyServerSnapshot, value); }},
    runMode: { capability: 'automation:write', execute(value) { mutable(); return guarded(runMode, applyRunResult, value); }},
    setModeEnabled: { capability: 'automation:write', execute(value) { mutable(); return guarded(setModeEnabled, applyServerSnapshot, value); }},
    ...
    start: { capability: 'automation:write', execute() { mutable(); return startAutomation(); }},
    stop: { capability: 'automation:write', execute() { mutable(); return stopAutomation(); }},
    runOnce: { capability: 'automation:write', execute() { mutable(); return runOnce(); }},
  ...
  function profile(root, mode, id, enabled) {
    ...
    if (mode === 'image-cuts') value.customCuts = getDraftCuts().map(cut => ({ label: cut.label, prompt: cut.prompt }));
  ...
  contract = createMenuContract({
    ...
    commands,
    render(view) {
      return renderAutomationView(view, { formatServerDriveMessage, renderModeEditor, renderPlacementPanel, makeDefaultProfile, disabledAttr, escapeHtml, escapeAttr });
    },
    bind(root) {
      const disposers = [];
      ...
      const bindMode = (mode, id) => {
        listen(disposers, one(root, `#serverSave${id}`), 'click', () => invokeAsync('saveProfile', profile(root, mode, id, false)));
        listen(disposers, one(root, `#serverRun${id}`), 'click', () => {
          const value = profile(root, mode, id, undefined);
          invokeAsync('runMode', value);
        });
        listen(disposers, one(root, `#serverAutoStart${id}`), 'click', () => invokeAsync('setModeEnabled', profile(root, mode, id, true)));
        listen(disposers, one(root, `#serverAutoStop${id}`), 'click', () => invokeAsync('setModeEnabled', profile(root, mode, id, false)));
        if (mode === 'image-cuts') {
          for (let index = 0; index < 10; index += 1) {
            for (const field of ['Label', 'Prompt']) {
              listen(disposers, one(root, `#serverCut${field}${id}_${index}`), 'input', event => updateDraftCut(index, field.toLowerCase(), event.target.value));
            }
          }
      ...
      for (const node of all(root, '[data-auto-output-refresh]')) listen(disposers, node, 'click', () => invokeAsync('refreshOutput', node.dataset.autoOutputRefresh || 'image-cuts'));
      for (const node of all(root, '[data-auto-output-preview]')) listen(disposers, node, 'click', () => {
      ...
      listen(disposers, one(root, '#connectDriveBtn'), 'click', () => invokeAsync('connectDrive', one(root, '#gdClientId')?.value || ''));
...
function renderAutomationView(view = {}, helpers) {
  const { formatServerDriveMessage, renderModeEditor, renderPlacementPanel, makeDefaultProfile, disabledAttr, escapeHtml, escapeAttr } = helpers;
